# Introduction  
## This project will be looking at College Basketball data from the 2015-2019 seasons. The data was collected from the following location: https://www.kaggle.com/andrewsundberg/college-basketball-dataset#cbb19.csv. The goal of this project will be to predict if a team will make it to the March Madness tournament, given the features provided. In order to determine if a user will make it to the March Madness Tournament, we will need to perform so exploratory analysis to better understand the data. We will predict based off a binary variable (1 = you made the March Madness Tournament and 0 = you did not)
...
## In this section we will be taking a look at the distributions of the different features. This will help give us a better understanding of the data and what type of feature engineering we may want to do. Before moving forward, we should check and address how we will deal with null values, convert any categorical variables to numerical values remove any unnecessary variables and produce some plots to better understand the distribution of the data in each column.
...
## Decision Tree
tree_ml <- train(SEED~., data = train_set, method = 'rpart', split = "gini", trControl = cv_param)
...
## We plot an ROC curve to see how much better our model is to someone randomly guessing. An ROC Curve plots the True Positive rate (predicted 1 that were true 1's) over our False Positive Rate (predicted 1 that were true 0's). Using our Random Forest model, we are able to predict which teams will make the NCAA March Madness Tournament with 96.35% accuracy. It appears that the most significant variables in predicting whether a team will make the NCAA March Madness Tournament are Games, Adjusted Offensive Efficiency, Adjusted Defensive Efficiency and a count of the number of teams in each conference. Other variables that showed statistical significance include BARTHAG and WAB, but had to be removed due to multicollinearity with the ADJOE feature. We are also able to see that we had a 89.29% True Positive Rate and a 93.94% True Negative Rate. Given the disproportinate number of teams that did not make the tournament, it makes sense that we are able to more accurately predict teams that do not make the tournmanent versus the teams that do.

# Conclusion
## Given that we had a training dataset of 1,750 teams over 5 years, we could have a slightly overfit model. Even though this dataset accounts for roughly 55,000+ NCAA basketball games, more data would definitely help us better evaluate the strength of this model. In conclusion, it appears that we did a great job cleaning up the dataset to predict whether or not a team will make the NCAA March Madness Tournament.